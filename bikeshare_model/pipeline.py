...
bikeshare_pipe = Pipeline(
    [
        ...
        (
            "weather_imputer",
            WeathersitImputer(weathersit_column=config.model_config.weathersit_column),
        ),
        # Outlier Handling
        (
            "outlier_handler",
        ...
        (
            "map_yr",
            Mapper(config.model_config.year_column, config.model_config.yr_mappings),
        ),
        (
            "map_mnth",
            Mapper(config.model_config.month_column, config.model_config.mnth_mappings),
        ),
        (
            "map_season",
            Mapper(
        ...
        (
            "map_weathersit",
            Mapper(
                config.model_config.weathersit_column,
        ...
        (
            "map_holiday",
            Mapper(
                config.model_config.holiday_column, config.model_config.holiday_mappings
            ),
        ),
        (
            "map_workingday",
            Mapper(
                config.model_config.workingday_column,
                config.model_config.workingday_mappings,
            ),
        ),
        (
            "map_hr",
            Mapper(config.model_config.hour_column, config.model_config.hr_mappings),
        ),
        # One-Hot Encoding
        (
            "weekday_encoder",
            WeekdayOneHotEncoder(weekday_column=config.model_config.weekday_column),
        ),
        # Drop Unused Columns
